        ...
        <div class="page-wrapper">
            <div class="page-content">
                ...
                <div class="card pb-3" style="width: 100%">
                    ...
                    <form action="{{url('updatesalaryperiod')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @foreach ($editsalary as $salary)
                        <input type="hidden" name="id" value="{{ $salary->id }}">
                        <select name="lang" aria-controls="example" class="form-select form-select-sm  mt-3">
                            <option value="{{ $salary->lang }}">
                                {{ $salary->lang }}</option>
                        </select>

                        <div class=" ps-4 pt-4">
                            <p class="h6">Select period</p>
                        </div>
                        ...
                        <div class="input-group mb-3 p-2">
                            <select name="salary_period" id="salary_period" required class="form-control form-select"
                                ...
                                @foreach ($get_edit_salary as $key => $get_edit_salary_list)
                                    <option  @if($get_edit_salary_list->salary_period == $salary->salary_period ) selected @endif value="{{ $get_edit_salary_list->salary_period }}">
                                        {{ $get_edit_salary_list->salary_period }}</option>
                                @endforeach
                        ...
                        <div class=" d-flex">
                            <div class="form-check ms-3 ">
                                <label class="form-check-label " for="is_default">
                                    <input type="radio" @if ($salary->is_default == 1) checked @endif name="is_default"
                            ...
                            <div class="form-check ms-3">
                                <label class="form-check-label" for="is_default">
                                    <input type="radio" @if ($salary->is_default == 0) checked @endif name="is_default"
                            ...
                            <div class=" d-flex ">
                                <d6iv class="form-check ms-4 ">
                                    <label class="form-check-label " name="is_active" id="is_active1">
                                        <input type="radio" @if ($salary->is_active == 1) checked @endif
                                        name="is_active"
                                ...
                                <div class="form-check ms-3">
                                    ...
                                    <label class="form-check-label" for="is_active2">
                                        <input type="radio" @if ($salary->is_active == 0) checked @endif
                                        name="is_active"
                        ...
                        <div class="ms-3 mt-4">
                            <button name="submit" type="submit" class="btn btn-secondary">
                                Update
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    class="bi bi-arrow-right-circle-fill ps-1" viewBox="0 0 16 16">
                                    <path
