            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($candidate_list as $key => $memb)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $memb->name }}</td>
                                    <td>{{ $memb->name }}</td>
                                    <td>{{ $memb->mobile_num }}</td>
                                    <td>{{ $memb->email }}</td>
                                    <td>{{ $memb->state_id }}</td>
                                    <td>{{ $memb->city_id }}</td>
                                    <td>
                                        <div class="col">
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <button type="button" class="btn btn-light"><a
                                                        href="{{ url('view_candidate', $memb->id) }}"><i class="lni lni-eye"></i>
                                                </button>
                                                <button type="button" class="btn btn-light"><a
                                                        href="{{ url('edit_candidate', $memb->id) }}"><i class='bx bx-edit me-0'></i>
                                                </button>
                                                <button type="button" class="btn btn-light"> <a
                                                        onclick="return confirm('Do you want to Confirm delete operation?')"
                                                        href="{{ url('/delete_candidate', $memb->id) }}"><i
