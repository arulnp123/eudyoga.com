              ...
              <div class="portlet light portlet-fit portlet-datatable bordered">
                  <div class="portlet-title d-flex justify-content-between">
                      <div class=" caption"> <i class="icon-settings font-dark"></i> <span
                              class="caption-subject font-dark sbold uppercase">Carrer Level</span> </div>
                      <div class="actions"> <a href="{{ url('add_carrer_level') }}"
                     ...
                     <table id="example2" class="table table-striped table-bordered">
                         ...
                         <tbody>
                             @foreach ($get_career_levels as $key => $career_levels_list)
                                 <tr>
                                     <td>{{ $key + 1 }}</td>
                                     <td>{{ $career_levels_list->lang }}</td>
                                     <td>{{ $career_levels_list->career_level }}</td>
                                     <td>
                                         <div class="col">
                                             <div class="btn-group" role="group" aria-label="Basic example">
                                                 <button type="button" class="btn btn-light"><a
                                                         href="{{ url('/view_carrer_level', $career_levels_list->id) }}"><i
                                                             class="lni lni-eye"></i>
                                                 </button>
                                                 <button type="button" class="btn btn-light"><a
                                                         href="{{ url('/edit_carrer_level', $career_levels_list->id) }}"><i
                                                             class='bx bx-edit me-0'></i>
                                                 </button>
                                                 <button type="button" class="btn btn-light"> <a
                                                         onclick="return confirm('Do you want to Confirm delete operation?')"
                                                         href="{{ url('/delete_carrer_level', $career_levels_list->id) }}"><i
