            ...
            <div class="page-content">
                ...
                <div class="card pb-3" style="width: 100%">
                    ...
                    <div class="card-body d-flex align-items-center ">
                        ...
                        <p class="card-text h4 ms-2">FAQS FORM</p>
                    </div>
                    ...
                    <form action="{{ url('save_faqs') }}" method="POST">
                        @csrf
                        ...
                        <select name="faq_question" id="faq_question" required class="form-control form-select mt-3">
                            ...
                            @foreach ($get_faq_question_list as $key => $get_faq_question_list)
                                <option value="{{ $get_faq_question_list->faq_question}}">
                                    {{ $get_faq_question_list->faq_question}}</option>
                            @endforeach
                        </select>
						</div>
                        ...
                        <div class="input-group mb-2 p-2">
                            <select name="faq_answer" id="faq_answer" required class="form-control form-select"
                                aria-label="Default select example">
                                <option value="">Answer</option>
                                @foreach ($get_faq_answer_list as $key => $get_faq_answer_list)
                                    <option value="{{ $get_faq_answer_list->faq_answer}}">
                                        {{ $get_faq_answer_list->faq_answer}}</option>
                                @endforeach
