          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="example2" class="table table-striped table-bordered">
                ...
                <tbody>
                  @foreach($packages as $key => $memb)
                  <tr>
                    <td>{{ $key + 1 }}</td>
                    <td>{{ $memb->package_title }}</td>
                    <td>{{ $memb->package_price}}</td>
                    <td>{{ $memb->package_num_days}}</td>
                    <td>{{ $memb->package_num_listings }}</td>
                    <td>{{ $memb->package_for }}</td>
                    <td>
                      <div class="col">
                        <div class="btn-group" role="group" aria-label="Basic example">
                          <button type="button" class="btn btn-light"><a href="{{ url('view_packages', $memb->id) }}"><i class="lni lni-eye"></i>
                          </button>
                          <button type="button" class="btn btn-light"><a href="{{ url('edit_packages', $memb->id) }}"><i class='bx bx-edit me-0'></i>
                          </button>
                          <button type="button"  class="btn btn-light"> <a onclick="return confirm('Do you want to Confirm delete operation?')"
                            href="{{ url('delete_packages', $memb->id) }}"><i class="lni lni-trash"></i></a></button>
